...
func NewRouter(db *sql.DB, session *session.Manager) *http.ServeMux {
	...
	mux.Handle("/api/v1/get_team_books/conditions", utils.CORS(handler.NewGetTeamBooksConditionHandler(session, db)))
	...
	mux.Handle("/api/v1/add_team_user", utils.CORS(handler.NewAddTeamUserHandler(session, db)))
	mux.Handle("/api/v1/remote_team_user", utils.CORS(handler.NewRemoveTeamUserHandler(session, db)))
	return mux
}
